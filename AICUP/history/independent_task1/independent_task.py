...
class ActionModel(nn.Module):
    def __init__(self, num_tokens_per_feature, n_act, emb_dim=16, hidden=128, num_layers=3, dropout=0.2, bidirectional=True):
        super().__init__()
...
class PointModel(nn.Module):
    def __init__(self, num_tokens_per_feature, n_pt, emb_dim=16, hidden=128, num_layers=1, dropout=0.2, bidirectional=True):
        super().__init__()
        self.num_features = len(num_tokens_per_feature)
        self.embs = nn.ModuleList([
            nn.Embedding(n + 1, emb_dim, padding_idx=PAD_TOKEN) for n in num_tokens_per_feature
    ...
    def forward(self, X, lengths):
        ...
        # 建立 Padding Mask 用於 Attention (B, 1, T)
        mask = (X[:, :, 0] != PAD_TOKEN).unsqueeze(1)
        
        # 透過 Attention 捕捉全局脈絡與最後一拍的關聯特徵
        o_attn = self.attn(o, mask)
...
def main(args):
    ...
    model_A = ActionModel(num_tokens_per_feature, n_act, emb_dim=args.emb, hidden=args.hidden,
                          num_layers=max(2, args.layers + 1), dropout=args.drop, bidirectional=args.bidirectional).to(device)
                          
    # Point 模型引入了特殊的 Attention 機制
    model_P = PointModel(num_tokens_per_feature, n_pt, emb_dim=args.emb, hidden=args.hidden,
                         num_layers=max(1, args.layers), dropout=args.drop, bidirectional=args.bidirectional).to(device)
